...
namespace KDV.CeusDL.Model.BT.Test
{
    ...
    public class BTModelTest
    {
        ...
        public void TestBTModel_BySimpleFileWithConfig()
        {
            ...
            var fileName = @"..\..\..\..\Test\Data\interface_demo.4.ceusdl".Replace('\\', System.IO.Path.DirectorySeparatorChar);
        ...
        {
            // Daten einlesen aus Datei ohne Config-Section
            var fileName = @"..\..\..\..\Test\Data\interface_demo.2.ceusdl".Replace('\\', System.IO.Path.DirectorySeparatorChar);
            var data = new ParsableData(System.IO.File.ReadAllText(fileName), fileName);
            var p = new FileParser(data);
            var result = p.Parse();            
            var model = new CoreModel(result);
        ...
        {
            // Daten einlesen aus Datei ohne Config-Section
            var fileName = @"..\..\..\..\Test\Data\interface_demo.5.ceusdl".Replace('\\', System.IO.Path.DirectorySeparatorChar);
            var data = new ParsableData(System.IO.File.ReadAllText(fileName), fileName);
            var p = new FileParser(data);
            var result = p.Parse();            
            var model = new CoreModel(result);
        ...
        {
            // Daten einlesen aus Datei ohne Config-Section
            var fileName = @"..\..\..\..\Test\Data\interface_demo.6.ceusdl".Replace('\\', System.IO.Path.DirectorySeparatorChar);
            var data = new ParsableData(System.IO.File.ReadAllText(fileName), fileName);
            var p = new FileParser(data);
            var result = p.Parse();            
            var model = new CoreModel(result);
